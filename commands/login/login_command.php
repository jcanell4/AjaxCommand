...
//        $this->types['p'] = abstract_command_class::T_STRING;
    ...
    protected function _run() {
        ...
        $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::LOGIN_INFO,
				$response));	//afegir si és login(true) o logout(false)

        $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::SECTOK_DATA,
				getSecurityToken()));
        
        if($response["loginResult"]){
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
                  array("type" => AjaxCmdResponseHandler::JSINFO,
		    "value" => $this->modelWrapper->getJsInfo(),
                )));                  
            
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::HTML_TYPE, 
					$this->modelWrapper->getLoginPageResponse()));
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
					  array("type" => AjaxCmdResponseHandler::CHANGE_WIDGET_PROPERTY,
							"id" => "exitButton", 
            ...
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
					  array("type" => AjaxCmdResponseHandler::CHANGE_WIDGET_PROPERTY,
							...
							"propertyValue" => false)));      
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
                      array("type" => AjaxCmdResponseHandler::RELOAD_WIDGET_CONTENT,
							"id" => "tb_index")));
			//elimina, si existe, la pestaña 'desconectat'
			$logout = $this->modelWrapper->getLogoutPageResponse();
			$ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
					array("type" => AjaxCmdResponseHandler::REMOVE_WIDGET_CHILD,
							"id" => $logout['id'])));
        }
		else{
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
            ...
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
                      array("type" => AjaxCmdResponseHandler::RELOAD_WIDGET_CONTENT,
							"id" => "tb_index")));
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::COMMAND_TYPE, 
                      array("type" => AjaxCmdResponseHandler::REMOVE_ALL_WIDGET_CHILDREN,
							"id" => "bodyContent")));
            $ret->add(new BasicJsonGenerator(AjaxCmdResponseHandler::HTML_TYPE, 
